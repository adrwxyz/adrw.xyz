...
As a long term solution, in EdgeRouter you can go to `Services > DHCP Server > LAN > Actions > View Leases` and then `Map Static IP` for the device that the Unifi Controller is running. Once it is assigned a static IP, you can set that IP in the `Details > Unifi Controller` box to prevent future adoption problems long term.
